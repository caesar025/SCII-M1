...
CONTAINS
  ...
  FUNCTION Rmanu(u,N,NQ,D,t,whichflux,vis,dt) result(solution)
    ...
    INTEGER                                                                    :: ierr,num_procs,i,id,num_elem !num_procs=#Processoren ierr=Errorflag
    REAL(KIND=RP)                                                              :: per_proc
    INTEGER,DIMENSION(MPI_STATUS_SIZE)                                         ::stat
    REAL(KIND=RP),DIMENSION(:,:,:,:,:,:,:),allocatable                         ::usub,uBoundL,uBoundR
    ...
    if(id==0.And.num_procs.LE.nq) then
      ...
      call MPI_SEND(u(:,:,(num_procs-1)*floor(per_proc),:,:,N+1,:),nq*nq*(N+1)**2*5,MPI_REAL,num_procs-1,0,MPI_COMM_WORLD,ierr)
    ...
    SELECT CASE(vis)
      ...
      CASE('VI')
        ...
        call computeGradient(usub,n,nq,num_elem,D,dux,duy,duz)
    ...
    END SELECT
    call MPI_SEND(usub,size(usub),MPI_REAL,0,0,MPI_COMM_WORLD,ierr)
    if (allocated(usub)) deallocate(usub)
    if (allocated(uBoundL)) deallocate(uBoundL)
    if (allocated(uBoundR)) deallocate(uBoundR)
    if(id==0) then
      do i=0,num_procs-2
        call MPI_RECV(u(:,:,i*floor(per_proc)+1:(i+1)*floor(per_proc),:,:,:,:),nq*nq*floor(per_proc)*(n+1)**3*5,MPI_REAL,&
          i,MPI_ANY_TAG,MPI_COMM_WORLD,stat,ierr)
      enddo
      call MPI_RECV(u(:,:,(num_procs-1)*floor(per_proc)+1:nq,:,:,:,:),nq*nq*(nq-(num_procs-1)*floor(per_proc))*(n+1)**3*5,&
        MPI_REAL,num_procs-1,MPI_ANY_TAG,MPI_COMM_WORLD,stat,ierr)
    endif
    call MPI_Finalize(ierr)
  END FUNCTION Rmanu
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
